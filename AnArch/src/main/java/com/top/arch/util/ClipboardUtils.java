...
public final class ClipboardUtils {
    ...
    public static void copyText(final CharSequence text) {
        ClipboardManager cm = (ClipboardManager) AnUtils.getApp().getSystemService(Context.CLIPBOARD_SERVICE);
        //noinspection ConstantConditions
        cm.setPrimaryClip(ClipData.newPlainText(AnUtils.getApp().getPackageName(), text));
    }
    ...
    public static CharSequence getText() {
        ...
        if (clip != null && clip.getItemCount() > 0) {
            CharSequence text = clip.getItemAt(0).coerceToText(AnUtils.getApp());
            if (text != null) {
                return text;
            }
        }
