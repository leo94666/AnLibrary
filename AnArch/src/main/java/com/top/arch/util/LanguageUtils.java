...
public class LanguageUtils {
    ...
    private static void applyLanguageReal(final Locale locale,
        ...
        updateAppContextLanguage(destLocal, new AnUtils.Consumer<Boolean>() {
            @Override
    ...
    public static Locale getAppContextLanguage() {
        return getContextLanguage(AnUtils.getApp());
    }
     ...
     */
    public static void updateAppContextLanguage(@NonNull Locale destLocale, @Nullable AnUtils.Consumer<Boolean> consumer) {
        pollCheckAppContextLocal(destLocale, 0, consumer);
    }

    static void pollCheckAppContextLocal(final Locale destLocale, final int index, final AnUtils.Consumer<Boolean> consumer) {
        Resources appResources = AnUtils.getApp().getResources();
        Configuration appConfig = appResources.getConfiguration();
        Locale appLocal = getLocal(appConfig);

        setLocal(appConfig, destLocale);

        AnUtils.getApp().getResources().updateConfiguration(appConfig, appResources.getDisplayMetrics());
    ...
    static void applyLanguage(final Activity activity) {
        ...
        updateConfiguration(AnUtils.getApp(), destLocal);
    }
