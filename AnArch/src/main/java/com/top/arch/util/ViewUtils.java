...
public class ViewUtils {
    ...
    public static boolean isLayoutRtl() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                primaryLocale = AnUtils.getApp().getResources().getConfiguration().getLocales().get(0);
            } else {
                primaryLocale = AnUtils.getApp().getResources().getConfiguration().locale;
            }
    ...
    public static View layoutId2View(@LayoutRes final int layoutId) {
        LayoutInflater inflate =
                (LayoutInflater) AnUtils.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
