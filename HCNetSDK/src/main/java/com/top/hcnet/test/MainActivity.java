...
import com.top.hcnet.jna.HCSadpSDKByJNA;
import com.top.hcnet.jna.HCSadpSDKJNAInstance;
import com.top.hcnetsdk.R;
...
public class MainActivity extends Activity {
	...
	private int g_dwSearchNum = 0;
	private int g_dwRecvNum = 0;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//开始搜索
		 ...
		 //刷新
		 refreshbn = (Button) findViewById(R.id.refreshbn);
		 refreshbn.setOnClickListener(new RefreshClickListener());
		...
		//停止搜索
		 ...
		 Export.setOnClickListener(new ExportClickListener());
		 ...
		 //搜索测试
		 SearchTest = (Button) findViewById(R.id.test);
		 SearchTest.setOnClickListener(new SearchTestClickListener());
	}
	...
	private DeviceFindCallBack_V40 getFindDeviceCallback_V40()
	{
		DeviceFindCallBack_V40 oDeviceFindCbf_V40 = new DeviceFindCallBack_V40()
		{
			public void fDeviceFindCallBack_V40(SADP_DEVICE_INFO_V40 lpDeviceInfo_V40)
			{
				...
				msg.what = 1;
				msg.obj = "[" + g_dwSearchNum + " : " + g_dwRecvNum + "]" + "Callback ipv4: " + strIpv4 + " szMac :" + strMac + " iResult:" + lpDeviceInfo_V40.struSadpDeviceInfo.iResult + "dwSDKOverTLSPort:" + lpDeviceInfo_V40.dwSDKOverTLSPort + "\n"+"  http port: "+lpDeviceInfo_V40.struSadpDeviceInfo.dwPort;
...
//				int iRet = SADPJNAInstance.getInstance().SADP_ActivateDevice("DS-2CD2Q10FD-IW20140314AAWR455578130", "hik12345");//194
	...
	//导出文件
	public class ExportClickListener implements OnClickListener {
		...
		public void onClick(View arg0) {
			...
			try
			{
				...
				//设备序列号
				HCSadpSDKByJNA.BYTE_ARRAY sDevSerialNO = new HCSadpSDKByJNA.BYTE_ARRAY("DS-2CD2622FWD-IZS20180312BBWR681619114".length() + 1);
				System.arraycopy("DS-2CD2622FWD-IZS20180312BBWR681619114".getBytes(), 0, sDevSerialNO.byValue, 0, "DS-2CD2622FWD-IZS20180312BBWR681619114".length());
				sDevSerialNO.write();
				
				//命令码
				int dwCommand = 2; //获取重置密码加密串
				
				//输入结构体
				Pointer lpInBuffer = null;
				
				//输出结构体
				HCSadpSDKByJNA.SADP_ENCRYPT_STRING struEncryptString = new	HCSadpSDKByJNA.SADP_ENCRYPT_STRING();
				Pointer lpStruEncryptString = struEncryptString.getPointer();
				
				int bRet = HCSadpSDKJNAInstance.getInstance().SADP_GetDeviceConfig(sDevSerialNO.getPointer(), dwCommand, 
						lpInBuffer, 0, lpStruEncryptString, struEncryptString.size());
				
				if (bRet == 0)
				{
					Message msg = new Message();
					msg.what = 1;
					msg.obj = "获取重置密码加密串  errorcode" + m_sadp.SADP_GetLastError() + "\n";
					mHandler.sendMessage(msg);
				}
				else if (bRet == 1)
				{
					...
					Message msg = new Message();
					msg.what = 1;
					msg.obj = "修改成功[" + struEncryptString.szEncryptString.toString() + "]\n";
					mHandler.sendMessage(msg);
					Log.i(TAG, "szEncryptString: " + new String(struEncryptString.szEncryptString)) ;	
				}
	...
	public class ResetClickListener implements OnClickListener {
		...
		public void onClick(View arg0) {
			...
			try
			{
				...
				//输入结构体
				HCSadpSDKByJNA.SADP_RESET_PARAM struResetParam = new HCSadpSDKByJNA.SADP_RESET_PARAM();
				
				//赋值给szCode是  自己输入OA转换后的安全码
				System.arraycopy("2e7c4d65".getBytes(), 0, struResetParam.szCode, 
						0, "2e7c4d65".length());
				
				//还可以是导入文件       和输入安全码二选一
//				System.arraycopy("文件路径".getBytes(), 0, struResetParam.szAuthFile, 
//						0, "文件路径".length());
				
				
				System.arraycopy("hik12345".getBytes(), 0, struResetParam.szPassword, 
						0, "hik12345".length());
				struResetParam.byEnableSyncIPCPW = 0;
				struResetParam.write();
				
				int bRet = HCSadpSDKJNAInstance.getInstance().SADP_ResetPasswd(sDevSerialNO.getPointer(), struResetParam.getPointer());
				
				if (bRet == 0)
				{
					Message msg = new Message();
					msg.what = 1;
					msg.obj = "重置密码失败  errorcode" + m_sadp.SADP_GetLastError() + "\n";
					mHandler.sendMessage(msg);
				}
				else if (bRet == 1)
				{		
	...
	//搜索设备压力测试
	public class SearchTestClickListener implements OnClickListener {
		...
		public void onClick(View arg0) {
			...
			try
			{
				...
				m_sadp = new Sadp();
		 		...
		 		boolean bRet = m_sadp.SADP_Start_V40(g_fPlaybackCallBack_V40);
		 		//关闭SDK的自动搜索
		 		m_sadp.SADP_SetAutoRequestInterval(0);
		 		try
		 		{
		 		    Thread.sleep(10000);
		 		}catch (Exception e ){
